...
namespace CTRFramework
{
    public class CtrMesh : IRead
    {
        ...
        public Vector3 scale = new Vector3(4096f,4096f,4096f);
            ...
            foreach (var d in drawList)
            {
                ...
                if (stripLength >= 2)
                {
                    ...
                    for (int z = 3 - 1; z >= 0; z--)
                    {
                        ...
                        if (d.texIndex != 0)
                            v.uv = Vector2b.ToVector2(tl[d.texIndex - 1].normuv[z]);
        ...
        public static CtrMesh FromRawData(string name, List<Vector3f> vertices, List<Vector4b> colors, List<Vector3i> faces)
        {
            ...
            {
                Color cl = new Color(c.W / 255.0f, c.Z / 255.0f, c.Y / 255.0f, c.X / 255.0f);
                
                cc.Add(new Vector4b((byte)(cl.r*255.0f), (byte)(cl.g*255.0f), (byte)(cl.b*255.0f), 0));
            }
        ...
        public void ExportTextures(string path, Tim vram = null)
        {
            ...
            if (tl.Count > 0)
            {
                ...
                {
                    Tim timTexture = vram.GetTimTexture(tex);
                    timTexture.Save(Path.Combine(texturepath, $"{tex.Tag()}.png"));
                }
            }
        }
